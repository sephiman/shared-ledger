...
import type { CompoundingPeriod, InterestType, LoanStatus } from "../../api/loans";

// TypeScript port of the backend LoanBalanceCalculator. Kept in parity via a shared
...
function effectiveEnd(loan: CalcLoan, asOf: Date): Date {
  if (loan.closedDate) {
    const closed = parseDate(loan.closedDate);
...
export function computeOutstanding(loan: CalcLoan, payments: CalcPayment[], asOfDate: string): OutstandingResult {
  const start = parseDate(loan.startDate);
  const end = effectiveEnd(loan, parseDate(asOfDate));
  ...
  if (loan.interestType === "none") {
    return computeNoInterest(loan, live);
  }

  const rate = loan.annualInterestRate ? new Decimal(loan.annualInterestRate).div(ONE_HUNDRED) : ZERO;

  const events: Event[] = [];
  if (loan.interestType === "compound") {
    const period: CompoundingPeriod = loan.compoundingPeriod ?? "monthly";
  ...
  let principal = new Decimal(loan.principalAmount);
  let accrued = ZERO;
  let cursor = start;
...
function computeNoInterest(loan: CalcLoan, payments: CalcPayment[]): OutstandingResult {
  let principal = new Decimal(loan.principalAmount);
  const allocations: Allocation[] = [];
...
export function previewSplit(
  ...
  const before = computeOutstanding(loan, existingPayments, proposedDate);
  const synthetic: CalcPayment = { id: SYNTHETIC_ID, paymentDate: proposedDate, amount: proposedAmount };
  const after = computeOutstanding(loan, [...existingPayments, synthetic], proposedDate);
