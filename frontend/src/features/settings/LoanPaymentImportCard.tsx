...
import { useExecuteLoanPayments, usePreviewLoanPayments, type ExecuteResult, type PreviewSummary } from "@/api/import";
...
export function LoanPaymentImportCard() {
  ...
  const previewMut = usePreviewLoanPayments(household.householdId);
  const executeMut = useExecuteLoanPayments(household.householdId);
  ...
  return (
    <Card>
      <CardHeader>
        <p className="font-medium">{t("import.loan_payments")}</p>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t("import.loan_payments_description")}</p>
        <CsvFormatHelp dataset="loan_payments" />
      </CardHeader>
      <CardBody className="space-y-3">
        ...
        {preview && !result && <PreviewPanel preview={preview} dataset="loan_payments" />}
