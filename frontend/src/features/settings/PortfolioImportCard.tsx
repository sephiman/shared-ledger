...
import { useExecutePortfolio, usePreviewPortfolio, type ExecuteResult, type PreviewSummary } from "@/api/import";
...
export function PortfolioImportCard() {
  ...
  const previewMut = usePreviewPortfolio(household.householdId);
  const executeMut = useExecutePortfolio(household.householdId);
  ...
  return (
    <Card>
      <CardHeader>
        <p className="font-medium">{t("import.portfolio")}</p>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t("import.portfolio_description")}</p>
        <CsvFormatHelp dataset="portfolio" />
      </CardHeader>
      <CardBody className="space-y-3">
        ...
        {preview && !result && <PreviewPanel preview={preview} dataset="portfolio" />}
