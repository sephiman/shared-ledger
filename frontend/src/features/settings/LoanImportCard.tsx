...
import { useExecuteLoans, usePreviewLoans, type ExecuteResult, type PreviewSummary } from "@/api/import";
...
export function LoanImportCard() {
  ...
  const previewMut = usePreviewLoans(household.householdId);
  const executeMut = useExecuteLoans(household.householdId);
  ...
  return (
    <Card>
      <CardHeader>
        <p className="font-medium">{t("import.loans")}</p>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t("import.loans_description")}</p>
        <CsvFormatHelp dataset="loans" />
      </CardHeader>
      <CardBody className="space-y-3">
        ...
        {preview && !result && <PreviewPanel preview={preview} dataset="loans" />}
