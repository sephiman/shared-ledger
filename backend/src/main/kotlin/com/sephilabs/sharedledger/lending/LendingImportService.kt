...
@Service
class LendingImportService(
    private val lendings: LendingRepository,
) {

    private val log = LoggerFactory.getLogger(LendingImportService::class.java)
    ...
    fun execute(householdId: UUID, input: InputStream, importer: User): ExecuteResult {
        log.info("Lending import started: household={} importer={}", householdId, importer.id)
        ...
        for (row in parsed.validRows) {
            ...
            val lending = Lending(
                ...
                status = row.status,
                closedDate = if (row.status == LendingStatus.active) null else row.startDate,
        ...
        log.info("Lending import finished: inserted={} skipped={} total={}", inserted, totalSkipped, parsed.totalRows)
    ...
    private fun parseAndValidate(householdId: UUID, input: InputStream): ParsedFile {
        ...
        val validRows = mutableListOf<ParsedLendingRow>()
        ...
        for ((idx, raw) in parsed.rows.withIndex()) {
            ...
            val status: LendingStatus = if (statusRaw.isBlank()) LendingStatus.active
            else runCatching { LendingStatus.valueOf(statusRaw) }.getOrNull() ?: run {
                rowErrors += RowError(rowNo, "IMPORT_STATUS_INVALID", "status", raw["status"])
                LendingStatus.active
            }

            if (rowErrors.isEmpty()) {
                validRows += ParsedLendingRow(
                    rowNumber = rowNo,
        ...
        val existingKeys = lendings.findAllByHouseholdId(householdId)
    ...
    private fun rowSummary(row: ParsedLendingRow): String =
    ...
    private data class ParsedLendingRow(
        val rowNumber: Int,
    ...
    private inner class ParsedFile(
        ...
        val validRows: MutableList<ParsedLendingRow>,
