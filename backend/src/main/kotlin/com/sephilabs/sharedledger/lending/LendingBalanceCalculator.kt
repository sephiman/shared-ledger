...
object LendingBalanceCalculator {
    ...
    fun compute(lending: Lending, payments: List<LendingPayment>, asOfDate: LocalDate): Outstanding {
        val effectiveEnd = effectiveEnd(lending, asOfDate)
        ...
        if (lending.interestType == InterestType.none) {
            return computeNoInterest(lending, livePayments)
        }

        val rate = lending.annualInterestRate
            ?.divide(ONE_HUNDRED, MC)
            ?: BigDecimal.ZERO

        val events = buildEvents(lending, livePayments, effectiveEnd)

        var principal = lending.principalAmount
        var accrued: BigDecimal = BigDecimal.ZERO
        var cursor = lending.startDate
    ...
    private fun effectiveEnd(lending: Lending, asOfDate: LocalDate): LocalDate {
        val closed = lending.closedDate
        return if (closed != null && closed.isBefore(asOfDate)) closed else asOfDate
    }

    private fun buildEvents(lending: Lending, payments: List<LendingPayment>, end: LocalDate): List<Event> {
        val list = mutableListOf<Event>()
        if (lending.interestType == InterestType.compound) {
            val period = lending.compoundingPeriod ?: CompoundingPeriod.monthly
            var anchor = nextCapAfter(lending.startDate, period)
    ...
    private sealed class Event {
        ...
        data class Payment(val payment: LendingPayment) : Event() {
