...
- Fields: occurrence date, amount (positive, except refunds — see below), direction (income or expense), category, optional description, who created it, who last updated it.
...
Money coming back for a past purchase — a returned jacket, a reimbursed train ticket, a partial rebate —
is recorded as a **refund: an expense with a negative amount**, dated when the money actually came back and
optionally linked to the purchase it nets. Booking it as income (the old `income.reimbursements` habit)
inflates income and leaves the category still showing the full spend; a negative expense instead **nets the
category and the month the money returned to**, keeps the savings rate honest, and never rewrites the
original purchase.

- **Why it needs no special handling downstream.** Every aggregation sums signed amounts by direction, so a
  negative expense flows correctly through monthly totals, category breakdowns, budgets, savings rate, cost
  of living and the FIRE spending base on its own. €100 spent and €30 back reads as €70 everywhere.
- **Three ways to record one.**
  - **Pending inbox → "Mark as refund"** on an income-direction item (money coming back reads as a credit at
    the bank). The dialog optionally searches the household's earlier expenses to link the original,
    prefills its category, and confirms the item as a negative expense on its own date and amount — linked
    through the same join table as any other confirm.
  - **Transactions list → "Register refund"** on any confirmed expense, for money returned without a bank
    row behind it (cash back at the till). The original is fixed; date and amount are entered by hand.
  - **The edit form → "Is refund"**, which is also the supported way to convert historical
    `income.reimbursements` rows: edit the row, switch it to expense, pick the real category, toggle the
    flag, negate the amount and optionally link the original. It is an ordinary edit, so the row keeps its
    id, its bank link and its audit trail.
- **Category.** A refund is validated like any expense and defaults to the linked purchase's category. Filing
  it elsewhere is allowed but the dialog says so plainly, since then *that* category is the one netted.
- **Guards.** A refund is always an expense with a negative amount, and can only point at a live, non-refund
  expense of the same household — a refund of a refund is refused, as is turning an already-refunded
  purchase into a refund. Refunds of income are out of scope: money going back out is an ordinary expense.
  Over-refunding is a **non-blocking warning** (partial and repeated refunds are normal), shown when the
  refunds linked to a purchase would add up to more than was paid.
- **In the list.** Refund rows show a green amount with a *Refund* badge and a link to the purchase they
  net; the purchase itself shows how much has come back ("−€30.00 refunded"). The direction filter treats
  refunds as the expenses they are, and a **Refunds only** chip narrows the list to them.
- Telegram rides the ordinary transactions toggle: a creation card for a new refund, an update card when an
  existing row is converted in the edit form.
...
- **Editing a trade**: a pencil on any trade row of the expanded holding opens the same form as *Add lot* / *Register sale*, prefilled with that trade's values, inline where it was clicked. **Every field is editable, including the type** — a buy can become a sale and back — because the save is an **atomic replace validated by a full replay**, not a patch: the holding's entire ledger is replayed with the new values in place, and if any sale ends up uncovered at any point in that replayed history (including the edited trade itself, once it is a sale) the save is **rejected naming the conflicting sale's date** and *nothing* persists — exactly the validation a fresh trade would face. Changing the trade date **re-resolves the frozen trade-time FX rate for the new date**: the frozen-at-trade-time rule follows the date, it does not pin the rate the trade was created with. Every derived figure — cost basis, realized P&L, XIRR, net invested, the evolution and ROI curves — recomputes from the replay exactly as it does for any other ledger change; nothing is special-cased. Switching the type shows a **non-blocking inline notice** (*"You are converting a buy into a sale"*) so a mis-click cannot slip through silently, while the save still just runs the normal validation. Edits are audited (`updated_by_user_id` / `updated_at`) and push a Telegram message as an *update* on the existing portfolio-trades toggle. An edited trade exports to CSV like any other, so the round-trip is unaffected.
...
- **Closed and dust positions leave the table, not the maths.** A holding is hidden from the Portfolio table when its **current market value is under €1** (`DUST_HOLDING_MAX_VALUE_EUR`) — one rule that covers a fully closed position (valued at exactly zero once net quantity reaches zero) and an exchange-rounding remnant of 8e-12 units alike. The criterion is **money, never quantity**: a tiny quantity of an expensive asset is real money and stays. An **unpriced** open holding is never hidden — its value is unknown, not small. A **Show closed** toggle (*Mostrar cerradas*) in the table header reveals them inline, sorted as usual, composes with the asset-type filter, and is remembered per browser (a view preference for this table, like the theme, not a Settings entry). Hiding is **display-only and always disclosed**: the count above the table states what it left out — *"4 holdings (+3 closed)"*, no suffix when everything is visible — while the summary cards (invested, current value, P&L, XIRR), the weights, Allocation, Evolution, the snapshot prefill and the CSV export all keep working over every holding. A portfolio where everything is closed says so and points at the toggle instead of reading as an empty table.
- **Every column sorts, and the weight follows the filter.** Clicking a header of the holdings table orders the rows by that column: **symbol** opens A→Z, every money column opens on the biggest position, and a second click reverses it — the caret and `aria-sort` state which. An **unpriced** holding sinks to the bottom whichever way the column points (no price is not a small price), ties fall back to the symbol so two equal rows never swap places between renders, and until a header is clicked the table keeps the order the API sent. The **Weight** column is a share of *what the filter left on the table*: pick an asset type and the percentages re-base on that class and still add up to 100 %, just like the totals above them. Hiding closed/dust rows never moves a weight — the shares are computed before that display-only step.
- **Views**: a **Portfolio** tab (totals for invested / current value / unrealized & realized P&L / total return plus a money-weighted return (XIRR) card, a per-holding table with its lots — closed and sub-€1 holdings hidden behind the **Show closed** toggle, every column sortable — and an asset-type filter that re-totals per class, weights included); an **Allocation** tab (pie charts by holding, by asset class, by crypto, by stock, and by latest snapshot); and an **Evolution** tab (daily market value with invested and realized/unrealized overlays, a **ROI (TWR)** chart — time-weighted return that chains daily market returns between trades, so contributions and withdrawals don't count as performance — plus asset-class / holding / time-range filters, shown above the snapshot-based net-worth evolution).
...
- **Mark as refund** (per item, under "More", on **income-direction rows only**): a credit that is really
  money coming back for a past purchase. Instead of income, it is confirmed as a **negative expense** on the
  item's own date and amount, netting the category it left from (see *Transactions → Refunds*). The dialog
  optionally searches earlier expenses to link the original and prefills its category; the category is
  required either way. Linked through the join table like any confirm, and **no rule is learned** — a
  refund's counterparty says nothing about what the household usually spends there. Single-item only.
...
Header (exact column names, any order): `date,direction,category_code,amount,description,created_at,updated_at,is_refund,refund_of_key`.
...
| `amount` | yes | Decimal with up to two decimals (e.g. `1234.56`), positive — except on a refund row, where it is negative. Comma decimal separators are also accepted. |
...
| `is_refund` | no | `true` or `false` (blank means `false`). A refund is money coming back for a past purchase: `direction` must be `expense` and the amount negative. |
| `refund_of_key` | no | Only on a refund row: the purchase it nets, named as `date\|direction\|category_code\|amount\|description`. |

**Files exported before refunds existed** — the seven-column header without `is_refund` and
`refund_of_key` — are still accepted, and every row in them is read as an ordinary transaction.

Because ids are not exported, a refund names its original by that natural key (the amount inside the key
uses a dot: it is an identifier, not a displayed number). On import the link is resolved against the
purchases in the same file first — originals are inserted before refunds, so row order doesn't matter — and
then against the household's existing transactions. If the purchase can't be found, or the key is stale
because the original was edited after the export, the refund is **imported without the link** and the
preview reports how many; it is never a row error.
...
2026-01-31,income,income.salary,2850.00,January payroll,,,false,
2026-02-03,expense,groceries.groceries,42.17,Mercadona,,,false,
2026-02-05,expense,outings.restaurants,28.50,Sushi,,,false,
2026-02-19,expense,groceries.groceries,-12.00,Mercadona return,,,true,2026-02-03|expense|groceries.groceries|42.17|Mercadona
```
