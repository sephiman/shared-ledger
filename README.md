...
1. **Deploy with `ENABLE_BANKING_APP_ID` still set.** Migration `V030` adds
   ...
   re-linking. The stamp runs in application code, not in the migration: a migration that
   interpolates a deployment-time value breaks outright on any value containing a quote, and makes
   the same migration mean different things per environment.
