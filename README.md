...
- **P&L percentages** (Portfolio tab totals and the Home Portfolio tile) each use their own denominator: **unrealized %** over the FIFO cost basis of open lots, **realized %** over the FIFO cost basis of sold lots across the whole history, and **total return %** over the total capital deployed (open + sold cost basis). Total return is a return-on-cost figure — money that is sold and reinvested deliberately counts in both terms; it is not time-weighted and not XIRR (the TWR chart covers that). A percentage shows "—" whenever its denominator is zero (e.g. realized % before any sale), never a fake 0%, and hovering a percentage shows the exact division behind it.
